...
async def _generate_command(query: str, shell: str) -> str:
    """Call Gemini via LangChain and return the raw command string."""
...
async def generate(req: CommandRequest):
    ...
    # 2️⃣  LLM fallback
    try:
        command = await _generate_command(req.query, shell)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"LLM error: {e}")

    # 3️⃣  Store result in cache + MongoDB
